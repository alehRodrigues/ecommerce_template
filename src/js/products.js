...
export const getData = new Promise((resolve, reject) => {
    ...
    if (window.localStorage.getItem('data')) {
        ...
        resolve(fetch('https://makeup-api.herokuapp.com/api/v1/products.json?product_type=nail_polish')
        .then(Resp => Resp.json())
        .then(products => window.localStorage.setItem('data', JSON.stringify(products)))
        .then(()=>{return JSON.parse(window.localStorage.getItem('data'))})
        )
    }
...
export const p = new Promise(resolve => {
    ...
    resolve(fetch('https://makeup-api.herokuapp.com/api/v1/products/156.json')
    .then(R=>R.json())
    .then((product)=>{
